...
output_dir = "/mnt/c/Users/joesc/OneDrive - University of Waterloo/MES_Thesis_Data/TSX_ESG_Embeddings_3"
csv_output_path = "/mnt/c/Users/joesc/OneDrive - University of Waterloo/MES_Thesis_Data/classification_results_3.csv"
...
# Load pre-trained FINBERT ESG model and tokenizer
model_name = 'yiyanghkust/finbert-esg'
model = BertModel.from_pretrained(model_name)
tokenizer = BertTokenizer.from_pretrained(model_name)

# Load or calculate embeddings for GRI and SASB
gri_embeddings_path = os.path.join(output_dir, "gri_embeddings.npy")
sasb_embeddings_path = os.path.join(output_dir, "sasb_embeddings.npy")

if os.path.exists(gri_embeddings_path) and os.path.exists(sasb_embeddings_path):
    gri_embeddings = np.load(gri_embeddings_path)
    sasb_embeddings = np.load(sasb_embeddings_path)
else:
    gri_text = read_text_from_file("/mnt/c/Users/joesc/OneDrive - University of Waterloo/MES_Thesis_Data/TSX_ESG_Clean/Consolidated_GRI_Standards_clean.txt")
    sasb_text = read_text_from_file("/mnt/c/Users/joesc/OneDrive - University of Waterloo/MES_Thesis_Data/TSX_ESG_Clean/SASB_Standards_Combined_clean.txt")
    
    gri_chunks = chunk_text(gri_text)
    sasb_chunks = chunk_text(sasb_text)
    
    gri_embeddings = get_embeddings_for_chunks(gri_chunks, model, tokenizer)
    sasb_embeddings = get_embeddings_for_chunks(sasb_chunks, model, tokenizer)
    
    np.save(gri_embeddings_path, gri_embeddings)
    np.save(sasb_embeddings_path, sasb_embeddings)
    print(f"GRI and SASB embeddings saved to {output_dir}")
...
for txt_file in unclassified_files:
    ...
    if text:
        ...
        embedding_file = os.path.join(output_dir, f"{os.path.basename(txt_file)}_embeddings.npy")
        ...
        plot_file = os.path.join(output_dir, f"{ticker}_{year}_combined_plot.png")
